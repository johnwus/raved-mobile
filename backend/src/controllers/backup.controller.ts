...
import { backupService, BackupOptions, ExportOptions } from '../services/backup.service';
...
export const backupController = {
  ...
  async createBackup(req: Request, res: Response) {
    try {
      ...
      if (type === 'postgresql') {
        ...
        const path = await backupService.backupPostgreSQL(options);
        metadata = {
          id: `pg_${Date.now()}`,
          type: 'postgresql',
        ...
        // MongoDB only backup
        const path = await backupService.backupMongoDB(options);
        metadata = {
          id: `mongo_${Date.now()}`,
          type: 'mongodb',
          databases: ['mongodb'],
        ...
        // Full system backup
        metadata = await backupService.createFullBackup(options);
      }
  ...
  async restoreBackup(req: Request, res: Response) {
    try {
      ...
      await backupService.restoreFromBackup(backupId, targetDatabases);
  ...
  async exportUserData(req: Request, res: Response) {
    try {
      ...
      const exportPath = await backupService.exportUserData(options);
  ...
  async requestDataDeletion(req: Request, res: Response) {
    try {
      ...
      // Notify admins
      const admins = await pgPool.query(`
  ...
  async processDataDeletion(req: Request, res: Response) {
    try {
