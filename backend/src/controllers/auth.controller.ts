...
import { smsTemplates } from '../services/sms-templates.service';
    ...
    try {
        ...
        if (identifier.includes('@')) {
            ...
            userResult = await pgPool.query(
                'SELECT id, username, email, phone, password_hash, first_name, last_name, sms_two_factor_enabled, phone_verified FROM users WHERE email = $1 AND deleted_at IS NULL',
                [identifier.toLowerCase()]
            );
        } else if (identifier.startsWith('@')) {
            // Username login
            userResult = await pgPool.query(
                'SELECT id, username, email, phone, password_hash, first_name, last_name, sms_two_factor_enabled, phone_verified FROM users WHERE username = $1 AND deleted_at IS NULL',
                [identifier.substring(1)]
            );
        } else {
            // Phone login
            userResult = await pgPool.query(
                'SELECT id, username, email, phone, password_hash, first_name, last_name, sms_two_factor_enabled, phone_verified FROM users WHERE phone = $1 AND deleted_at IS NULL',
                [identifier]
            );
        }
        ...
        if (user.sms_two_factor_enabled && user.phone && user.phone_verified) {
            ...
            await redis.setex(codeKey, 300, twoFactorCode); // 5 minutes

            // Send SMS 2FA code
            await smsService.sendTwoFactorCode(user.phone, twoFactorCode);
...
export const resetPassword = async (req: Request, res: Response) => {
    try {
        ...
        try {
            const userResult = await pgPool.query(
                'SELECT phone, sms_security_alerts_enabled FROM users WHERE id = $1',
                [user_id]
            );
            ...
            if (userResult.rows.length > 0) {
                ...
                if (user.phone && user.sms_security_alerts_enabled) {
                    await smsService.sendSecurityAlert(user.phone, 'password changed');
                }
            }
        } catch (smsError) {
...
export const sendSMSVerification = async (req: Request, res: Response) => {
    try {
        ...
        // Get user details
        const userResult = await pgPool.query(
            'SELECT phone, phone_verified, sms_two_factor_enabled FROM users WHERE id = $1',
            [userId]
        );
        ...
        await redis.setex(codeKey, 600, verificationCode); // 10 minutes
...
export const requestSMSPasswordReset = async (req: Request, res: Response) => {
    try {
        ...
        // Find user by phone
        const userResult = await pgPool.query(
            'SELECT id, first_name FROM users WHERE phone = $1 AND deleted_at IS NULL',
            [phone]
        );
        ...
        await redis.setex(codeKey, 600, resetCode);
...
export const resetPasswordWithSMS = async (req: Request, res: Response) => {
    try {
...
export const enableSMSTwoFactor = async (req: Request, res: Response) => {
    try {
        const userId = req.user.id;
        ...
        // Enable SMS 2FA
        await pgPool.query(
            'UPDATE users SET sms_two_factor_enabled = true, updated_at = CURRENT_TIMESTAMP WHERE id = $1',
            [userId]
        );
...
export const disableSMSTwoFactor = async (req: Request, res: Response) => {
    try {
        const userId = req.user.id;

        // Disable SMS 2FA
        await pgPool.query(
            'UPDATE users SET sms_two_factor_enabled = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1',
            [userId]
        );
...
export const sendSMSTwoFactorCode = async (req: Request, res: Response) => {
    try {
        const { userId } = req.body; // This would come from login attempt

        // Get user details
        const userResult = await pgPool.query(
            'SELECT phone, sms_two_factor_enabled FROM users WHERE id = $1',
            [userId]
        );
...
export const verifySMSTwoFactorCode = async (req: Request, res: Response) => {
    try {
...
export const refresh = async (req: Request, res: Response) => {
    try {
        ...
        // Get user details
        const userResult = await pgPool.query('SELECT id, username, email, first_name, last_name FROM users WHERE id = $1', [userId]);
        ...
        await redis.setex(`refresh_token:${user.id}`, 7 * 24 * 60 * 60, newRefresh);
...
export const updateUserLanguagePreferences = async (req: Request, res: Response) => {
    try {
        const userId = req.user.id;
