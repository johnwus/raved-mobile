...
export const connectionService = {
  sendFollowRequest: async (followerId: string, followingId: string) => {
    ...
    const existingConnectionResult = await pgPool.query(
  ...
  getPendingFollowRequests: async (userId: string) => {
  ...
  approveFollowRequest: async (requestId: string, userId: string) => {
    const result = await pgPool.query(`
      UPDATE connections 
    ...
    await notificationsController.createNotification(
      ...
      `${actorName} approved your follow request`,
      userId,
      { type: 'follow_request_approved' }
    );
  },

  rejectFollowRequest: async (requestId: string, userId: string) => {
    const result = await pgPool.query(`
  ...
  blockUser: async (blockerId: string, blockedId: string) => {
    ...
    const connectionCheck = await pgPool.query(`
