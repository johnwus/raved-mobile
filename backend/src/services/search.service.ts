...
const buildSearchCondition = (parsedQuery: any, fields: string | string[], fallbackValue?: string) => {
  ...
  const addConditionGroup = (value: string, operator: 'ILIKE' | 'NOT ILIKE') => {
...
const searchServiceInternal = {
  ...
  getSuggestions: async (query: string, type: string = 'all', limit: number = 10) => {
    ...
    if (type === 'all' || type === 'users') {
      const userQuery = `
        ...
        LIMIT $2
      `;
      const userResult = await pgPool.query(userQuery, [`${query}%`, limit]);
    ...
    if (type === 'all' || type === 'items') {
      const itemQuery = `
        ...
        LIMIT $2
      `;
      const itemResult = await pgPool.query(itemQuery, [`${query}%`, limit]);
      itemResult.rows.forEach(row => {
        if (row.name) suggestions.push(row.name);
        if (row.category) suggestions.push(row.category);
        if (row.brand) suggestions.push(row.brand);
      });
    }
    ...
    if (type === 'all' || type === 'events') {
      const eventQuery = `
        ...
        LIMIT $2
      `;
      const eventResult = await pgPool.query(eventQuery, [`${query}%`, limit]);
      eventResult.rows.forEach(row => {
        if (row.title) suggestions.push(row.title);
        if (row.category) suggestions.push(row.category);
        if (row.location) suggestions.push(row.location);
      });
    }
  ...
  // Search analytics
  trackSearch: async (userId: string, query: string, type: string, resultCount: number, filters: any) => {
    const queryText = `
      INSERT INTO search_analytics (user_id, query, search_type, result_count, filters, searched_at)
      VALUES ($1, $2, $3, $4, $5, NOW())
    `;
    await pgPool.query(queryText, [userId, query, type, resultCount, JSON.stringify(filters)]);
  },

  getPopularSearches: async (limit: number = 20) => {
    const query = `
  ...
  // Saved searches
  saveSearch: async (userId: string, name: string, query: string, type: string, filters: any) => {
    const queryText = `
      INSERT INTO saved_searches (user_id, name, query, search_type, filters, created_at)
      VALUES ($1, $2, $3, $4, $5, NOW())
      RETURNING id
    `;
    const result = await pgPool.query(queryText, [userId, name, query, type, JSON.stringify(filters)]);
  ...
  deleteSavedSearch: async (userId: string, searchId: number) => {
    const query = `
      UPDATE saved_searches
      SET deleted_at = NOW()
      WHERE id = $1 AND user_id = $2
    `;
    await pgPool.query(query, [searchId, userId]);
  },

  // Export search results
  exportSearchResults: async (results: any, format: 'csv' | 'json' = 'json') => {
    if (format === 'json') {
...
export const searchService = {
  ...
  advancedSearch: async (
   ...
   const parsedQuery = parseSearchQuery(q);
    ...
    const results: { users: any[]; posts: any[]; items: any[]; events: any[]; faculties: any[] } = {
      users: [],
      posts: [],
      items: [],
      events: [],
    ...
    if (type === 'all' || type === 'users') {
      ...
      const userParams: (string | number)[] = [...userCondition.params, q.trim().toLowerCase()];
      if (filters.faculty) userParams.push(filters.faculty);
      userParams.push(limit);
      userParams.push(offset);

      const userResult = await pgPool.query(userQuery, userParams);
    ...
    if (type === 'all' || type === 'items') {
      ...
      const itemParams: (string | number)[] = [...itemCondition.params, q.trim().toLowerCase()];
      ...
      const itemSortMap: { [key: string]: string } = {
      ...
      const itemResult = await pgPool.query(itemQuery, itemParams);
      ...
      sellers.rows.forEach(s => {
        sellerMap[s.id] = {
          ...
          avatarUrl: getAvatarUrl(s.avatar_url, s.id)
        };
      });
    ...
    if (type === 'all' || type === 'events') {
      ...
      const eventParams: (string | number)[] = [...eventCondition.params, q.trim().toLowerCase()];
      ...
      const eventSortMap: { [key: string]: string } = {
        'relevance': 'relevance_score DESC',
        'newest': 'created_at DESC',
      ...
      const eventResult = await pgPool.query(eventQuery, eventParams);
      ...
      const organizerMap: { [key: string]: any } = {};
    ...
    if (type === 'all' || type === 'faculties') {
      ...
      `;
      const facultyParams: (string | number)[] = [...facultyCondition.params, limit, offset];
      const facultyResult = await pgPool.query(facultyQuery, facultyParams);
