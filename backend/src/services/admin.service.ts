...
export const adminService = {
  getReports: async (status: string, type: string, page: number, limit: number) => {
  ...
  reportContent: async (reporterId: string, contentType: string, contentId: string, reason: string, description?: string) => {
  ...
  resolveReport: async (reportId: string, action: string, notes: string, adminId: string) => {
    ...
    switch (action) {
      case 'remove_content':
        await adminService.removeReportedContent(report);
        break;
      case 'suspend_user':
        await adminService.suspendUser(report.reported_user_id, 'Content violation', 7); // 7 days suspension
        break;
      case 'warn':
        await adminService.sendWarningToUser(report.reported_user_id, report.reason, notes);
        break;
    }
  ...
  removeReportedContent: async (report: any) => {
    switch (report.content_type) {
      ...
      case 'item':
        await pgPool.query(
          'UPDATE store_items SET status = $1, deleted_at = CURRENT_TIMESTAMP WHERE id = $2',
          ['removed', report.content_id]
        );
        break;
      case 'event':
        await pgPool.query(
  ...
  suspendUser: async (userId: string, reason: string, days: number) => {
  ...
  sendWarningToUser: async (userId: string, reason: string, notes?: string) => {
    await Notification.create({
      userId,
      type: 'system',
  ...
  getPlatformStatistics: async (period: string) => {
  ...
  getUserManagementList: async (search?: string, role?: string, status?: string, page: number = 1, limit: number = 50) => {
    const offset = (page - 1) * limit;
    ...
    return result.rows.map(row => ({
      ...
      avatarUrl: getAvatarUrl(row.avatar_url, row.id),
      faculty: row.faculty,
  ...
  getThemeAnalytics: async (period: string) => {
    let startDate = new Date();
    switch (period) {
      case '7d':
        startDate.setDate(startDate.getDate() - 7);
    ...
    const themeStats = await pgPool.query(`
      SELECT
        theme_preference,
        COUNT(*) as count,
    ...
    const dailyChanges = await pgPool.query(`
      SELECT
  ...
  getThemeUsageStats: async () => {
    const totalUsers = await pgPool.query('SELECT COUNT(*) as count FROM users WHERE deleted_at IS NULL');
    const themeUsers = await pgPool.query('SELECT COUNT(*) as count FROM users WHERE theme_preference IS NOT NULL AND deleted_at IS NULL');
    const darkModeUsers = await pgPool.query('SELECT COUNT(*) as count FROM users WHERE dark_mode_preference = true AND deleted_at IS NULL');

    const themeBreakdown = await pgPool.query(`
  ...
  setDefaultTheme: async (themeId: string, darkMode: boolean, adminId: string) => {
