...
import {
  ...
  ABTest,
  ABTestResult
} from '../models/postgres/analytics-event.model';
...
export class AnalyticsService {
  ...
  async updateMetric(metric: Omit<AnalyticsMetric, 'id' | 'created_at'>): Promise<void> {
    try {
      await pgPool.query(`
  ...
  async getRealtimeMetrics(timeRangeMinutes: number = 60): Promise<any> {
    try {
  ...
  async logUserActivity(activity: Omit<UserActivityLog, 'id' | 'created_at'>): Promise<void> {
    try {
      await pgPool.query(`
  ...
  async generateReport(
    ...
    try {
      ...
      const report: Omit<AnalyticsReport, 'id' | 'created_at'> = {
  ...
  private async getEventStats(startDate: Date, endDate: Date): Promise<any> {
    const result = await pgPool.query(`
      SELECT
        event_type,
        event_category,
  ...
  private async getPageStats(startDate: Date, endDate: Date): Promise<any> {
    const result = await pgPool.query(`
      SELECT
        page_url,
        COUNT(*) as views,
  ...
  private async getDeviceStats(startDate: Date, endDate: Date): Promise<any> {
    const result = await pgPool.query(`
      SELECT
        device_type,
        browser,
  ...
  private async getConversionStats(startDate: Date, endDate: Date): Promise<any> {
    const result = await pgPool.query(`
      SELECT
  ...
  async getVariantForUser(testName: string, userId: string): Promise<string> {
    try {
  ...
  private selectVariantByWeight(weights: number[]): number {
  ...
  async trackABTestResult(
    ...
    try {
      ...
      if (result.rowCount === 0) {
        logger.warn('No variant found for user in A/B test', { testName, userId });
      }
    } catch (error) {
  ...
  async cleanupOldData(): Promise<void> {
    try {
      ...
      logger.info(`Cleaned up ${result.rowCount} old analytics events`);
    } catch (error) {
