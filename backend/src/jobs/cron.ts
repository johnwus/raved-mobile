...
import { sendEmail } from '../utils/index'; // Assuming sendEmail is in utils
import { AnalyticsCronJobs } from './analytics-cron';
...
export function initializeBackgroundJobs() {
  ...
  cron.schedule('0 1 * * *', AnalyticsCronJobs.aggregateDailyMetrics);

  // Generate weekly reports every Monday at 2 AM
  cron.schedule('0 2 * * 1', AnalyticsCronJobs.generateWeeklyReports);

  // Generate monthly reports on 1st of month at 3 AM
  cron.schedule('0 3 1 * *', AnalyticsCronJobs.generateMonthlyReports);

  // Update real-time cache every 15 minutes
  cron.schedule('*/15 * * * *', AnalyticsCronJobs.updateRealtimeCache);

  // Clean up old analytics data monthly
  cron.schedule('0 4 1 * *', AnalyticsCronJobs.cleanupOldAnalyticsData);
  ...
  cron.schedule('*/5 * * * *', async () => {
    try {
      ...
      const usersWithQueues = await pgPool.query(`
      ...
      for (const user of usersWithQueues.rows) {
        try {
          await OfflineQueueService.processQueue(user.user_id);
          totalProcessed++;
        } catch (error) {
  ...
  cron.schedule('*/10 * * * *', async () => {
    try {
      ...
      const usersWithOfflineData = await pgPool.query(`
        SELECT DISTINCT user_id
        FROM offline_data
      ...
      for (const user of usersWithOfflineData.rows) {
        try {
          const result = await OfflineDataService.syncOfflineData(user.user_id);
  ...
  cron.schedule('0 1 * * *', async () => {
    try {
      ...
      const expiredCount = await OfflineDataService.cleanupExpiredData();

      // Clean up old resolved conflicts
      const conflictsCleaned = await SyncConflictService.cleanupResolvedConflicts();
      ...
      for (const user of usersWithQueues.rows) {
        const cleaned = await OfflineQueueService.cleanupOldItems(user.user_id);
  ...
  cron.schedule('0 * * * *', async () => {
    try {
      ...
      const usersWithFailedSyncs = await pgPool.query(`
        SELECT DISTINCT user_id
        FROM offline_queues
        WHERE status = 'failed'
      ...
      for (const user of usersWithFailedSyncs.rows) {
        try {
          const retried = await OfflineQueueService.retryFailedItems(user.user_id);
...
async function awardRankingPrizes(topUsers: any[], period: 'weekly' | 'monthly') {
  try {
...
async function cleanupExpiredBackups() {
  try {
    ...
    const deletedCount = await backupService.deleteExpiredBackups();
