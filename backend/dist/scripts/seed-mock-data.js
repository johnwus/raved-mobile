...
async function seedAdminUser() {
    ...
    try {
        ...
        const adminPassword = await (0, auth_utils_1.hashPassword)('admin123');
    ...
    try {
      ...
      `, [conn.followerId, conn.followingId, conn.createdAt]);
        ...
        for (const userId of userIds) {
            const updateClient = await database_1.pgPool.connect();
            try {
...
async function seedPosts() {
    try {
        ...
        await post_model_1.Post.deleteMany({ userId: { $in: userIds } });
        let postCount = 0;
        for (const userId of userIds) {
        ...
        for (const userId of userIds) {
            const postCountResult = await post_model_1.Post.countDocuments({ userId });
            await database_1.pgPool.query('UPDATE users SET posts_count = $1 WHERE id = $2', [postCountResult, userId]);
        }
...
async function seedStories() {
    try {
        ...
        await story_model_1.Story.deleteMany({ userId: { $in: userIds } });
        let storyCount = 0;
        for (const userId of userIds) {
...
async function seedLikesAndComments() {
    try {
        ...
        // Get all posts
        const posts = await post_model_1.Post.find({ deletedAt: null }).lean();
        if (posts.length === 0) {
        ...
        for (const post of posts) {
            ...
            for (const userId of shuffledUsers) {
                await like_model_1.Like.findOneAndUpdate({ userId, targetId: post._id.toString(), targetType: 'post' }, {
                    userId,
            ...
            // Update post counts
            const likesCount = await like_model_1.Like.countDocuments({ targetId: post._id.toString(), targetType: 'post' });
            const commentsCount = await comment_model_1.Comment.countDocuments({ postId: post._id.toString(), deletedAt: null });
            await post_model_1.Post.updateOne({ _id: post._id }, {
                $set: {
                    likesCount,
                    commentsCount
                }
...
async function seedAdminData() {
    try {
        ...
        // Get admin user ID
        const adminResult = await database_1.pgPool.query('SELECT id FROM users WHERE username = $1', ['admin']);
        ...
        const mockUsers = await database_1.pgPool.query('SELECT id FROM users WHERE username LIKE $1 LIMIT 5', ['mock_%']);
      ...
      `, [adminId, mockUserId]);
        ...
        const followerCount = await database_1.pgPool.query('SELECT COUNT(*) FROM connections WHERE following_id = $1 AND status = $2', [adminId, 'accepted']);
        const followingCount = await database_1.pgPool.query('SELECT COUNT(*) FROM connections WHERE follower_id = $1 AND status = $2', [adminId, 'accepted']);
        await database_1.pgPool.query(`
      UPDATE users SET
        followers_count = $2,
        following_count = $3,
    ...
    `, [adminId, parseInt(followerCount.rows[0].count), parseInt(followingCount.rows[0].count)]);
        ...
        for (const postData of adminPosts) {
            ...
            await database_1.pgPool.query('UPDATE users SET posts_count = posts_count + 1 WHERE id = $1', [adminId]);
        }
