...
async function initializeDatabase() {
    try {
        ...
        await (0, database_1.initializePostgresSchema)();
...
async function createIndexes() {
    ...
    for (const indexQuery of indexes) {
        try {
            await database_1.pgPool.query(indexQuery);
        }
        catch (error) {
...
async function seedInitialData() {
    try {
        ...
        const userCount = await database_1.pgPool.query('SELECT COUNT(*) FROM users');
        ...
        // Create admin user
        const adminPassword = await Promise.resolve().then(() => __importStar(require('../utils/auth.utils'))).then(m => m.hashPassword('admin123'));
