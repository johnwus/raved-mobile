...
exports.connectionsController = {
    ...
    getUserConnections: async (req, res) => {
        ...
        `, [userId]);
          ...
          `, [currentUserId, followerIds]);
            ...
            // Get total counts
            const followingCountResult = await database_1.pgPool.query('SELECT COUNT(*) as count FROM connections WHERE follower_id = $1 AND status = $2', [userId, 'accepted']);
            const followersCountResult = await database_1.pgPool.query('SELECT COUNT(*) as count FROM connections WHERE following_id = $1 AND status = $2', [userId, 'accepted']);
            res.json({
                success: true,
                connections: {
                    following,
    ...
    getConnectionSuggestions: async (req, res) => {
        try {
            const userId = req.user.id;
            const limit = parseInt(req.query.limit) || 10;
            // Get users with similar faculty, not already connected, not blocked
            const suggestions = await database_1.pgPool.query(`
        SELECT u.id, u.username, u.first_name, u.last_name, u.avatar_url, u.faculty, u.bio,
