...
exports.syncCommentCounts = exports.getFacultyPosts = exports.getPostStoreItem = exports.likeComment = exports.deleteComment = exports.getPostComments = exports.commentOnPost = exports.likePost = exports.getPost = exports.getTrendingPosts = exports.getPostSuggestions = exports.getFeed = exports.sharePost = exports.unsavePost = exports.savePost = exports.createPost = void 0;
...
const createPost = async (req, res) => {
    try {
        ...
        if (isForSale && saleDetails) {
            ...
            const storeInsert = await database_1.pgPool.query(`
            ...
            const salePost = new mongoose_1.Post({
                ...postData,
                saleDetails: {
...
const savePost = async (req, res) => {
    ...
    try {
        ...
        await mongoose_1.SavedPost.updateOne({ userId, postId }, { $setOnInsert: { userId, postId, createdAt: new Date() } }, { upsert: true });
        // Increment savesCount (best-effort)
        await mongoose_1.Post.updateOne({ _id: postId }, { $inc: { savesCount: 1 } });
...
const unsavePost = async (req, res) => {
    ...
    const { postId } = req.params;
    try {
        const result = await mongoose_1.SavedPost.deleteOne({ userId, postId });
...
const sharePost = async (req, res) => {
    ...
    try {
        ...
        await mongoose_1.Post.updateOne({ _id: postId }, { $inc: { sharesCount: 1 } });
        // Optionally persist share in Postgres 'share' table if present
        try {
            await database_1.pgPool.query('INSERT INTO shares (user_id, post_id, created_at) VALUES ($1, $2, CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING', [userId, postId]);
        }
        catch (e) {
...
const getPostSuggestions = async (req, res) => {
    try {
        const userId = req.user.id;
        const limit = parseInt(req.query.limit) || 10;
        const { FeedAlgorithmService } = await Promise.resolve().then(() => __importStar(require('../services/feed-algorithm.service')));
        ...
        const enrichedSuggestions = suggestions.map(post => ({
            ...
            saleDetails: post.isForSale && post.saleDetails ? { storeItemId: post.saleDetails.storeItemId } : undefined,
...
exports.getPostSuggestions = getPostSuggestions;
// Get trending posts
    ...
    const enrichedPosts = posts.map(post => {
        ...
        return {
            ...
            saleDetails: post.isForSale && post.saleDetails ? {
                ...
                negotiable: false,
                storeItemId: post.saleDetails.storeItemId,
            } : undefined,
            location: post.location,
            brand: post.brand,
        ...
        if (existingLike) {
            ...
            if (post && post.userId !== userId) {
                try {
                    ...
                    const actorInfo = await database_1.pgPool.query('SELECT id, username, first_name, avatar_url FROM users WHERE id = $1', [userId]);
...
const commentOnPost = async (req, res) => {
    try {
        ...
        const userInfo = await database_1.pgPool.query('SELECT id, username, first_name, last_name, avatar_url FROM users WHERE id = $1', [comment.userId]);
        ...
        (0, post_moderation_middleware_1.moderateCommentAfterCreation)(comment._id.toString(), userId, postId, text).catch((err) => console.error('Async comment moderation failed:', err));
        // Post-processing: Send notifications asynchronously
        setTimeout(async () => {
            try {
                // Get actor (commenter) info for notifications
                const actorInfo = await database_1.pgPool.query('SELECT id, username, first_name, last_name, avatar_url FROM users WHERE id = $1', [userId]);
                const actor = actorInfo.rows[0];
...
const getPostComments = async (req, res) => {
    try {
        ...
        const topLevelComments = await mongoose_1.Comment.find({
            postId,
        ...
        const replies = await mongoose_1.Comment.find({
            postId,
        ...
        const commentLikes = await mongoose_1.Like.find({
            userId: req.user.id,
        ...
        const enrichedComments = topLevelComments.map(comment => ({
            ...
            createdAt: comment.createdAt,
            replies: (repliesMap[comment._id.toString()] || []).map((reply) => ({
                id: reply._id.toString(),
                user: userMap[reply.userId],
                text: reply.text,
                timeAgo: (0, utils_1.getTimeAgo)(reply.createdAt),
        ...
        const totalTopLevelComments = await mongoose_1.Comment.countDocuments({
            postId,
        ...
        try {
            const totalCommentCount = await mongoose_1.Comment.countDocuments({
                postId,
                deletedAt: null,
                isRemoved: { $ne: true }
            });
            if (totalCommentCount !== undefined) {
                await mongoose_1.Post.updateOne({ _id: postId }, { $set: { commentsCount: totalCommentCount } });
            }
        }
        catch (syncError) {
...
const deleteComment = async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        // Find the comment
        const comment = await mongoose_1.Comment.findById(commentId);
        if (!comment) {
        ...
        await mongoose_1.Comment.updateOne({ _id: commentId }, { deletedAt: new Date() });
...
const likeComment = async (req, res) => {
    try {
        ...
        if (existingLike) {
            // Unlike the comment
            await mongoose_1.Like.deleteOne({ _id: existingLike._id });
            await mongoose_1.Comment.updateOne({ _id: commentId }, { $inc: { likesCount: -1 } });
            action = 'unliked';
        }
        else {
            // Like the comment
...
const getPostStoreItem = async (req, res) => {
    try {
        const { postId } = req.params;
        // Ensure post exists
        const post = await mongoose_1.Post.findById(postId).lean();
        ...
        if (name) {
            baseQuery.push('AND name = $' + (params.length + 1));
            params.push(name);
        }
        if (price) {
            baseQuery.push('AND price = $' + (params.length + 1));
            params.push(price);
        }
        baseQuery.push('ORDER BY created_at DESC LIMIT 1');
        const exact = await database_1.pgPool.query(baseQuery.join(' '), params);
        if (exact.rows[0]?.id) {
            return res.json({ success: true, itemId: exact.rows[0].id });
        }
        // Fallback: fuzzy name match
        const fuzzy = await database_1.pgPool.query('SELECT id FROM store_items WHERE seller_id = $1 AND status = $2 AND deleted_at IS NULL AND similarity(name, $3) > 0.3 ORDER BY similarity(name, $3) DESC, created_at DESC LIMIT 1', [userId, 'active', name]).catch(() => ({ rows: [] }));
        if (fuzzy.rows?.[0]?.id) {
...
exports.getPostStoreItem = getPostStoreItem;
const getFacultyPosts = async (req, res) => {
    try {
        ...
        const usersResult = await database_1.pgPool.query('SELECT id FROM users WHERE LOWER(faculty) = LOWER($1) AND deleted_at IS NULL', [facultyName]);
 ...
 */
const syncCommentCounts = async (req, res) => {
    try {
        console.log('🔄 Starting comment count sync...');
        const posts = await mongoose_1.Post.find({ deletedAt: null });
        let fixedCount = 0;
        for (const post of posts) {
            const actualCommentCount = await mongoose_1.Comment.countDocuments({
                postId: post._id,
                deletedAt: null
            });
            if (actualCommentCount !== post.commentsCount) {
                console.log(`📍 Post ${post._id}: count mismatch ${post.commentsCount} -> ${actualCommentCount}`);
                await mongoose_1.Post.updateOne({ _id: post._id }, { $set: { commentsCount: actualCommentCount } });
                fixedCount++;
            }
        }
...
exports.syncCommentCounts = syncCommentCounts;
