...
exports.eventsController = {
    ...
    getEvents: async (req, res) => {
      ...
      `;
            ...
            const events = result.rows.map(event => ({
                ...
                timeAgo: (0, utils_1.getTimeAgo)(event.created_at)
            }));
            res.json({
                success: true,
                events,
    ...
    getEvent: async (req, res) => {
      ...
      `, [userId, eventId]);
            ...
            // Get attendees list
            const attendeesResult = await database_1.pgPool.query(`
    ...
    createEvent: async (req, res) => {
        try {
            ...
            const userId = req.user.id;
            const { title, description, date, time, location, maxAttendees, category, audience, image } = req.body;
            const result = await database_1.pgPool.query(`
        INSERT INTO events (
    ...
    updateEvent: async (req, res) => {
        try {
            ...
            const eventCheck = await database_1.pgPool.query('SELECT organizer_id FROM events WHERE id = $1 AND deleted_at IS NULL', [eventId]);
      ...
      `;
            values.push(eventId);
            const result = await database_1.pgPool.query(updateQuery, values);
            const event = result.rows[0];
            res.json({
                success: true,
    ...
    deleteEvent: async (req, res) => {
        try {
            ...
            await database_1.pgPool.query('UPDATE events SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1', [eventId]);
            res.json({
                success: true,
    ...
    toggleAttendance: async (req, res) => {
        try {
            ...
            const eventResult = await database_1.pgPool.query(`
      ...
      `, [userId, eventId]);
            ...
            if (isAttending) {
                // Remove attendance
                await database_1.pgPool.query('DELETE FROM event_attendees WHERE event_id = $1 AND user_id = $2', [eventId, userId]);
                await database_1.pgPool.query('UPDATE events SET current_attendees = current_attendees - 1 WHERE id = $1', [eventId]);
                res.json({
                    success: true,
                ...
                // Add attendance
                await database_1.pgPool.query('INSERT INTO event_attendees (event_id, user_id) VALUES ($1, $2)', [eventId, userId]);
                await database_1.pgPool.query('UPDATE events SET current_attendees = current_attendees + 1 WHERE id = $1', [eventId]);
                res.json({
                    success: true,
    ...
    getEventAttendees: async (req, res) => {
      ...
      `, [eventId, parseInt(limit), offset]);
            const attendees = result.rows.map(attendee => ({
                ...
                faculty: attendee.faculty,
                joinedAt: attendee.joined_at,
                timeAgo: (0, utils_1.getTimeAgo)(attendee.joined_at)
            }));
            res.json({
                success: true,
                attendees,
                pagination: {
                    page: parseInt(page),
                    limit: parseInt(limit),
                    hasMore: attendees.length === parseInt(limit)
                }
            });
        }
        catch (error) {
