...
exports.ShareAnalyticsService = void 0;
const database_1 = require("../config/database");
class ShareAnalyticsService {
    // Track share event
    static async trackShareEvent(shareId, eventType, userId, metadata) {
        const query = `
      INSERT INTO share_analytics_events (
        share_id, event_type, user_id, metadata, created_at
      ) VALUES ($1, $2, $3, $4, NOW())
    `;
        await database_1.pgPool.query(query, [
            shareId,
            eventType,
            userId || null,
    ...
    static async getContentShareAnalytics(contentType, contentId) {
        const query = `
      SELECT
        s.platform,
    ...
    static async getUserShareAnalytics(userId) {
        const query = `
      SELECT
        s.content_type,
        COUNT(DISTINCT s.id) as total_shares,
    ...
    static async getPlatformAnalytics(platform, dateRange) {
        let query = `
      SELECT
    ...
    static async getTopPerformingContent(limit = 10) {
        const query = `
      SELECT
        s.content_type,
        s.content_id,
    ...
    static async generateShareReport(dateRange) {
        const query = `
      SELECT
        s.platform,
        s.content_type,
    ...
    `;
        const result = await database_1.pgPool.query(query, [dateRange.start, dateRange.end]);
        return result.rows;
    }
}
exports.ShareAnalyticsService = ShareAnalyticsService;
exports.default = ShareAnalyticsService;
