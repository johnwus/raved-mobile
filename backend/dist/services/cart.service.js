...
exports.cartService = {
    addItemToCart: async (userId, itemId, quantity) => {
        const itemResult = await database_1.pgPool.query(`
    ...
    `, [itemId]);
        ...
        const existingCartItem = await database_1.pgPool.query(`
    ...
    updateCartItemQuantity: async (cartItemId, userId, quantity) => {
        if (quantity < 1) {
    ...
    clearCart: async (userId) => {
        await database_1.pgPool.query('DELETE FROM cart_items WHERE user_id = $1', [userId]);
    },
    saveItemToWishlist: async (userId, itemId) => {
        const itemResult = await database_1.pgPool.query(`
    ...
    `, [itemId]);
        ...
        const existing = await database_1.pgPool.query(`
    ...
    removeItemFromWishlist: async (userId, itemId) => {
        const result = await database_1.pgPool.query(`
