...
class DeepLinkingService {
    ...
    // Create deep link
    static async createDeepLink(contentType, contentId, platform, campaign, expiresAt, metadata) {
    ...
    // Resolve deep link
    static async resolveDeepLink(shortCode) {
        const query = `
    ...
    `;
        const result = await database_1.pgPool.query(query, [shortCode]);
        return result.rows[0] || null;
    }
    // Track deep link click
    static async trackDeepLinkClick(shortCode, userAgent, ipAddress, referrer) {
    ...
    static async getDeepLinkAnalytics(shortCode) {
        const query = `
      SELECT
        dl.click_count,
        dl.last_clicked_at,
    ...
    static generatePlatformDeepLink(contentType, contentId, platform) {
        const baseUrl = process.env.BASE_URL || 'https://yourapp.com';
        switch (platform) {
            case 'ios':
                // iOS Universal Links
    ...
    static async createFirebaseDynamicLink(contentType, contentId, socialTitle, socialDescription, socialImageUrl) {
    ...
    static async createBranchDeepLink(contentType, contentId, metadata) {
...
exports.default = DeepLinkingService;
